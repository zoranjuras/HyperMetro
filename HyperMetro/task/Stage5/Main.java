...
//        String filePath = "HyperMetro/task/src/metro/prague_w_time.json";
    ...
    private static List<WeightedNeighbour> getWeightedNeighbours(StationRef current, Map<String, DoublyLinkedList<Station>> mapOfLines) {
        ...
        if (index >= 0) {
            ...
            for (Station.Transfer transfer : station.getTransfers()) {
                weightedNeighbours.add(new WeightedNeighbour(new StationRef(transfer.getLine(), transfer.getStation()), 5));

            }

            if (index > 0) {
                Station prevStation = stations.get(index - 1);
                weightedNeighbours.add(new WeightedNeighbour(new StationRef(current.line(), prevStation.getName()), prevStation.getTime()));

            }

            if (index < stations.size() - 1) {
                Station nextStation = stations.get(index + 1);
                weightedNeighbours.add(new WeightedNeighbour(new StationRef(current.line(), nextStation.getName()), station.getTime()));
